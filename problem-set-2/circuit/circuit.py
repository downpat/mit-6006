...
class Transition:
    ...
    def is_valid(self):
        ...
        output.
        """
        return self.gate is not None and self.gate.output != self.new_output
...
class PriorityQueue:
    ...
    def parent(self, i):
        """Returns the parent index of the ith element"""
        return (i - 1) / 2 if i - 1 <= 0 else 0

    def left(self, i):
        """Returns the left child index of the ith element"""
        return i * 2 + 1
    ...
    def min_heapify(self, i):
        """Reposition the ith element in the heap"""
        smallest = i
        l = self.left(i)
        r = self.right(i)
        #print 'l:', l, 'r:', r, 'i:', i, 'queue length:', len(self.queue)
        if l < len(self.queue) and self.queue[l] < self.queue[smallest]:
            smallest = l
    ...
    def heap_decrease_key(self, i, key):
        #print 'Replacing', self.queue[i], 'with', key
        if key > self.queue[i]:
            raise ValueError('Key is larger than current key')
        self.queue[i] = key
        #print 'Queue after replace:', self.queue
    ...
    def append(self, key):
        ...
        #print 'Appending', key, 'to the queue'
        if key is None:
            raise ValueError('Cannot insert None in the queue')
        self.queue.append(Transition(Gate('inf', GateType('inf', TruthTable('inf', [0, 0]), 0)), 0, float('inf')))
        self.heap_decrease_key(len(self.queue) - 1, key)
        #print 'Queue after append', self.queue
    ...
    def pop(self):
        ...
        """
        #print 'Popping from the queue:', self.queue
        if len(self.queue) == 0:
            return None
        popped_key = self.queue.pop(0)
        if len(self.queue) > 0:
            self.queue.insert(0, self.queue.pop(-1))
            self.min_heapify(0)
        #print 'Popped element:', popped_key
